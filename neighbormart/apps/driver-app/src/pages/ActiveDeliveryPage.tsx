    ...
    <div className="p-5 space-y-4">
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-5 shadow-sm space-y-4">
        <div className="flex items-center justify-between">
          <span className="font-semibold text-gray-900">{active.order?.customer?.name}</span>
        ...
        <div className="flex items-start gap-3 p-3 bg-gray-50 rounded-xl">
          ...
          <div>
            <div className="font-medium text-gray-900 text-sm">{active.addressText || 'Delivery address'}</div>
            {active.addressLat && (
        ...
        {active.order?.customer?.phone && (
          <a href={`tel:${active.order.customer.phone}`}
            className="flex items-center gap-3 p-3 bg-blue-50 rounded-xl text-blue-700">
            <Phone size={18} />
            <span className="font-medium text-sm">{active.order.customer.phone}</span>
          </a>
        )}
      ...
      {showFail && (
        <div className="fixed inset-0 bg-black/50 flex items-end justify-center z-50">
          <div className="bg-white rounded-t-2xl p-5 w-full max-w-md space-y-4">
            <h3 className="font-bold text-gray-900">Report Failed Delivery</h3>
            <textarea
            ...
            <div className="flex gap-3">
              <button onClick={() => setShowFail(false)} className="flex-1 bg-gray-100 text-gray-700 rounded-xl py-3 font-medium text-sm">Cancel</button>
